...
class TestIOSSecurityRulesPatterns:
    ...
    def test_slack_webhook_pattern_exists(self):
        ...
        # Verify rule loads and has a pattern (actual webhook URLs blocked by GitHub push protection)
        assert rule.pattern is not None
        assert "hooks.slack.com" in rule.pattern.pattern

    def test_stripe_secret_pattern_exists(self):
        rule = self.rules["cred_stripe_secret"]
        # Verify rule loads and has a pattern (actual keys blocked by GitHub push protection)
        assert rule.pattern is not None
        assert "sk_live" in rule.pattern.pattern or "sk_test" in rule.pattern.pattern
