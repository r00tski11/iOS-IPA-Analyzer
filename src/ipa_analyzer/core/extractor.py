...
def create_extractor(input_path: Path) -> IPAExtractor | XCArchiveExtractor:
    """Factory function to create the appropriate extractor based on input type.

    Args:
        input_path: Path to an .ipa file or .xcarchive directory.

    Returns:
        An IPAExtractor or XCArchiveExtractor instance.
    ...
    if input_path.is_dir():
        raise InvalidIPAError(
            ...
            f"Expected a .xcarchive directory or .ipa file."
        )
    raise InvalidIPAError(
        f"Unsupported input: '{input_path.name}'. Provide a .ipa file or .xcarchive directory."
    )
